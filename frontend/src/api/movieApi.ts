...
export const getMovieDetail = async (movieId: number): Promise<MovieDetail> => {
  try {
    ...
    return {
      ...
      ott_providers: otts.map((ott: any) => ({
        // ✅ otts → ott_providers 변환
    ...
    return {
      ...
      title: "인터스텔라",
      overview:
        "세계 각국의 정부와 경제가 완전히 붕괴된 미래가 다가온다. 지구 대기권에서 극심한 먼지 폭풍이 일어나고, 결국 지구에서의 삶은 불가능하게 된다. 여전히 남아있는 자들을 위한 최후의 희망은 우주 저편에 살 수 있는 새로운 행성을 찾는 것이다. 지구의 미래를 짊어진 그들의 위대한 도전이 시작된다.",
      genres: ["SF", "드라마", "모험"],
      ...
      cast: [
        {
          ...
          character: "쿠퍼",
          profile_url:
            "https://image.tmdb.org/t/p/w185/cnqwv5Uz3d8c4TxVGxGkjOJuFPb.jpg",
        },
        {
          name: "앤 해서웨이",
          character: "아멜리아 브랜드",
          profile_url:
            "https://image.tmdb.org/t/p/w185/tLelacaCxfRFRpGLYkdEY3d1mrq.jpg",
        },
        {
          name: "제시카 차스테인",
          character: "머피 쿠퍼",
          profile_url:
            "https://image.tmdb.org/t/p/w185/vOFtVlCUyMMBXJ0RvJkS7lKoPVG.jpg",
        },
        {
          name: "마이클 케인",
          character: "브랜드 교수",
          profile_url:
            "https://image.tmdb.org/t/p/w185/bVZRMlpjTAO2pJK6v90buFgVbSW.jpg",
        },
        {
          name: "맷 데이먼",
          character: "맨 박사",
          profile_url:
            "https://image.tmdb.org/t/p/w185/ehwS5WvU5yL5vKcUEqbzGK8Fh8B.jpg",
        },
      ],
      ...
      ott_providers: [
        ...
        },
        {
          ...
          ott_name: "Disney+",
          ott_logo:
            "https://image.tmdb.org/t/p/original/dgPueyEdOwpQ10fjuhL2WYFQwQs.png",
...
export const postRecommendations = async (filters: {
  ...
  try {
    ...
    console.error(
      "영화 추천 API 호출 중 오류 (백엔드 연결 실패, 임시 데이터 사용):",
      error
    );
    ...
    const mockMovies: Movie[] = [
      // Algorithmic (필터 기반 추천) - 3개
      {
        ...
        poster:
          "https://image.tmdb.org/t/p/w500/gEU2QniE6E77NI6lCU6MxlNBvIx.jpg",
        description:
          "우주를 배경으로 펼쳐지는 감동적인 SF 대작. 시간과 공간을 초월한 사랑 이야기.",
        popular: false,
        watched: false,
      },
      {
    ...
    const filteredPopular = mockMovies.filter((m) => m.popular).slice(0, 3);

    // 필터 조건에 맞는 영화가 부족하면 모든 영화에서 채우기
    const allAlgorithmic =
      filteredAlgorithmic.length >= 3
        ? filteredAlgorithmic
