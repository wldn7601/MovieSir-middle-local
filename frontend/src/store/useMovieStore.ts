...
export const useMovieStore = create<MovieState>((set, get) => ({
  ...
  loadRecommended: async () => {
    ...
    if (!userId) {
      console.warn(
        "⚠️ 로그인하지 않음 - 임시 ID(0) 사용, 임시 데이터 반환 예정"
      );
    }
    ...
    try {
      ...
      console.log(
        "  - 맞춤추천 초기 표시:",
        initialRecommended.map((m) => m.title)
      );
      console.log(
        "  - 인기영화 초기 표시:",
        initialPopular.map((m) => m.title)
      );
  ...
  removeRecommendedMovie: (movieId) =>
    set((state) => {
      ...
      console.log(
        "  현재 표시 중:",
        state.recommendedMovies.map((m) => `${m.id}:${m.title}`)
      );
      ...
      // 1. 현재 표시 중인 영화에서 제거
      const newRecommended = state.recommendedMovies.filter(
        (m) => m.id !== movieId
      );
      ...
      // 4. 사용자가 장르를 선택한 경우, 해당 장르와 일치하는 영화만 필터링
      ...
      // 5. 다음 영화가 있으면 추가하고 shownIds 업데이트
      if (nextMovie) {
        console.log(
          "✅ 다음 영화로 채움:",
          nextMovie.title,
  ...
  removePopularMovie: (movieId) =>
    set((state) => {
      ...
      console.log(
        "  현재 표시 중:",
        state.popularMovies.map((m) => `${m.id}:${m.title}`)
      );
      ...
      // 3. 전체 목록에서 아직 표시되지 않은 영화 찾기
      const availableMovies = state.allPopularMovies.filter(
        (m) => !shownIds.includes(m.id)
      );
      console.log("  남은 인기영화:", availableMovies.length, "개");
      console.log(
        "  남은 영화 목록:",
        availableMovies.map((m) => m.title)
      );
